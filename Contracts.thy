...
type_synonym pre_contract = "(form\<times>(form list)) set"

value "simplify_emp_sepconj (foldr (\<lambda>(x,e) r. (f_eq x e) \<^emph> r) (zip [x,y] [X,Y]) Emp)"

typedef contract = "{(cs, (f,args))::pre_contract\<times>func | cs args f.
  \<forall>(P,Q)\<in>cs. quantifier_free P \<and> symbolic_heap P \<and> \<comment> \<open>P is a quantifier-free symbolic heap\<close>
  ...
  list_all (\<lambda>q. case strip_prenex q of (UQ, Qf \<^emph> Qeq) \<Rightarrow>
    ...
    quantifier_free Qf \<and> symbolic_heap Qf \<and> free_vars Qf \<inter> set args = {} \<and>
...
lift_definition sound :: "contract \<Rightarrow> bool" is
  "\<lambda>(cs, (f,args)). \<forall>(P,Qs) \<in> cs. \<forall>c c' F.
